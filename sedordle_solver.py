...
class Sedordle_Reinforcement_Learning():
	def __init__(self, dict_path, debug = False, num_game_boards = 16, daily = False, end_with_close = False):

		self.daily = daily

		self.quordle_game = Sedordle_Website_Game(ultra_instinct = True, daily = daily)
	...
	def execute_final_guesses(self, game_boards):
		...
		vocabs = [Sedordle_Reinforcement_Learning.vocab_filter_total(s, self.vocab) for s in states]
		curr = np.argmin([v.shape[0] for v in vocabs])
		game_boards.pop(curr)
		vocab_dist = Sedordle_Reinforcement_Learning.get_vocab_distribution(vocabs[curr])
		info_list = Sedordle_Reinforcement_Learning.word_information(vocabs[curr], vocab_dist, states[curr])
		new_guess = max(info_list, key = lambda x: x[1])
		max_info_guesses = sorted(info_list, key = lambda x: x[1], reverse = True)
		print([f"{Sedordle_Reinforcement_Learning.to_word(w[0])}: {w[1]:.2f}" for w in max_info_guesses[0:10]])
		new_word_guess = Sedordle_Reinforcement_Learning.to_word(new_guess[0])
	...
	def live_play_ultra(self):
		...
		while (not final_turns) and (not done):
			...
			if self.quordle_game.turn == 0:
				...
				vocab = Sedordle_Reinforcement_Learning.vocab_filter_total(state, self.vocab)
				zero_out_state = Sedordle_Reinforcement_Learning.get_zero_out_state(state)
				vocab_dist = Sedordle_Reinforcement_Learning.get_vocab_distribution(vocab)
				new_vocab_dist = vocab_dist * zero_out_state
				print(vocab.shape)
				full_info_list = Sedordle_Reinforcement_Learning.word_information(self.vocab, new_vocab_dist, state)
			else:
				for i in range(self.num_game_boards):
					print(i)
					...
					new_vocab_dist = vocab_dist * zero_out_state
					print(vocab.shape)
					info_list = Sedordle_Reinforcement_Learning.word_information(self.vocab, new_vocab_dist, state)
	...
	def get_initial_vocab(self, dict_path):
		...
		return Sedordle_Reinforcement_Learning.to_onehot(encoded_words_numpy)
	...
	def word_information(vocab, vocab_distribution, state):
		...
		for v in vocab:
			info_list.append(Sedordle_Reinforcement_Learning.information_gained(v, vocab_distribution, state))
	...
	def get_state(self, guesses):
		...
		sorted_updates = Sedordle_Reinforcement_Learning.sort_updates(all_update_guesses)
	...
	def vocab_filter_total(state, vocab):
		...
		for p in range(state.shape[0]):
			for l in range(state.shape[1]):
				vocab = Sedordle_Reinforcement_Learning.vocab_filter_cycle(vocab, state[p, l], p, l)
		return vocab
	

if __name__ == "__main__":
	dict_path = "/Users/nuowenlei/Desktop/Programming/Python/Wordle_cuz_why_not/five_letter_words.json"
	trisha = Sedordle_Reinforcement_Learning(dict_path, daily = True)
