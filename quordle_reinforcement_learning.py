...
class Quordle_Reinforcement_Learning():
	def __init__(self, dict_path, debug = False, num_game_boards = 4, daily = False, end_with_close = False):

		self.daily = daily

		self.quordle_game = Quordle_Website_Game(ultra_instinct = True, daily = daily)
	...
	def execute_final_guesses(self, game_boards):
		...
		vocabs = [Quordle_Reinforcement_Learning.vocab_filter_total(s, self.vocab) for s in states]
		curr = np.argmin([v.shape[0] for v in vocabs])
		game_boards.pop(curr)
		vocab_dist = Quordle_Reinforcement_Learning.get_vocab_distribution(vocabs[curr])
		info_list = Quordle_Reinforcement_Learning.word_information(vocabs[curr], vocab_dist, states[curr])
		...
		new_word_guess = Quordle_Reinforcement_Learning.to_word(new_guess[0])
	...
	def live_play_ultra(self):
		...
		while (not final_turns) and (not done):
			...
			if self.quordle_game.turn == 0:
				...
				vocab = Quordle_Reinforcement_Learning.vocab_filter_total(state, self.vocab)
				zero_out_state = Quordle_Reinforcement_Learning.get_zero_out_state(state)
				vocab_dist = Quordle_Reinforcement_Learning.get_vocab_distribution(vocab)
				new_vocab_dist = vocab_dist * zero_out_state
				print(vocab.shape)
				full_info_list = Quordle_Reinforcement_Learning.word_information(self.vocab, new_vocab_dist, state)
			else:
				for i in range(self.num_game_boards):
					print(i)
	...
	def get_initial_vocab(self, dict_path):
		...
		return Quordle_Reinforcement_Learning.to_onehot(encoded_words_numpy)
	...
	def word_information(vocab, vocab_distribution, state):
		...
		for v in vocab:
			info_list.append(Quordle_Reinforcement_Learning.information_gained(v, vocab_distribution, state))
	...
	def get_state(self, guesses):
		...
		sorted_updates = Quordle_Reinforcement_Learning.sort_updates(all_update_guesses)
	...
	def vocab_filter_total(state, vocab):
		...
		for p in range(state.shape[0]):
			for l in range(state.shape[1]):
				vocab = Quordle_Reinforcement_Learning.vocab_filter_cycle(vocab, state[p, l], p, l)
		return vocab
	

if __name__ == "__main__":
	dict_path = "/Users/nuowenlei/Desktop/Programming/Python/Wordle_cuz_why_not/five_letter_words.json"
	trisha = Quordle_Reinforcement_Learning(dict_path, daily = True)
